...
import glob
import h5py
#liu@1003 changed since there is sth wrong with util's loadmat
...
def getRecord(data, total=-1, pos=True, pose=False, facial=False):
    ...
    # input arg "total" is 10000, the maximum number of images to be evaluated
    if total == -1:
        total = data.getTotal()
    else:
...
#            self.ann = sio.loadmat(self.trainfile)["anno"]
#        else:
#            self.trainfile = "annotations/new_annotations_AFW.mat"
#            #self.ann = loadmat(self.trainfile)["Annotations"]
#            # directly get a structure called annotation
#            self.ann = sio.loadmat(self.trainfile)["Annotations"]
...
class WIDERFACE(VOC06Data):
    ...
    def __init__(self, select="all", cl="face_train.txt",
        ...
        self.useOldAnn = useOldAnn
        if useOldAnn:
            self.trainfile = "annotations\widerface_anno_e1-e3.mat"
            #self.ann = loadmat(self.trainfile)["anno"]
            self.ann = sio.loadmat(self.trainfile)["anno"]
        else:
            self.trainfile = "annotations\widerface_anno_e1-e3.mat"
